...
\begin{defn}[Control flow graph]
    Control flow graph is a graph on the set of basic blocks of the given procedure with edges specified by branches (represented by \li{goto}s, possibly in \li{if} statements and \li{switch} statements) in pairs $(\mtt{fromIdx}, \mtt{toIdx})$ and a special basic block called entry block which begins the procedure and is never branched to.
\end{defn}

\begin{defn}[Blocks table]
...
\begin{defn}[Blocks data]
    Blocks data is a mapping from each pair \linebreak \li{(blockIdx, varName)} to a triple \li{(reads before write, writes, requests alive)}. The first two values of the triples are filled in by the blockifier (they are given syntactically) and the last one by the subsequent live-range analysis, which we will describe \xxx{where}.

    This mapping, together with control flow graph, is then used in translation (\cref{translation}) for rewriting the code into SSA form required by LLVM, more specifically, for generating so-called ``phi nodes'' \xxx{ref ref ref}.
\end{defn}
...
After the blockifier builds the whole control flow graph and blocks data, live-range analysis (originally described by \cite{Aho2006compilers} as live-variable analysis, we use simplified version described by \cite{bednarek2008compilers}) measures live-ranges of all variables, gives errors for uninitialized variables (variables requested alive in the entry block) and warnings for unreachable basic blocks (basic blocks not in the dfs closure generated from the entry block).
...
In the modified \cmm{} language, we do not support existentials (as discussed in \ref{sys_defer}) and all polymorphic definitions have to be at top level (or they are easily representable as such: definitions of methods and their instances). This allows the solver to solve more eagerly, since there is no need for context levels, deferring the constraints only on two cases (see \cref{main_loop}).
...
\begin{lang}
  ...
  \begin{grammar}
    ...
    \otherformP{C?\ \mathbb{T}^\star}{Class constraint (the given types have to satisfy the given constraint - requires an instantiation of a proof)}
    \otherformP{C!\ \mathbb{T}^\star}{Class fact (the given types are known to satisfy the given constraint - provides the proof to $C?\ \mathbb{T}^\star$)}
...
\begin{defn}[Fresh type variable]
    \label{def:prefresh}
...
\midrule
  ...
  $cs \in C_{cs} \to \mathbb{W}^\star,\,C_{cs} \in 2^{\mbb C}$ & \textbf{Class schemes}\quad
  ...
  $cf \in C_{cf} \to \mathbb{W}^\star,\,C_{cf} \subseteq C_{cs}$ & \textbf{Class facts}\quad
...
\begin{defn}[Introducing a new variable (not necessarily fresh)]
    \label{introVar}
...
\end{defn}
\begin{defn}[Algorithm preparation step for a given constraint]
    \label{presteps}
...
\end{lstlisting}
    \end{minipage}
    For the specific ``mangled'' names given to the copies of \li{id}, see \cref{sec:mangling}.
    \end{center}
\end{listing}

Note that if there was no limit on monomorphization (observe step \ref{steptwo}), it could potentially continue forever (we usually choose the limit so it is safely bigger than the monomorphization depth of majority of practical programs; this is precedented, for example, by \cite{haskell2010}):
...
\begin{defn}[Name mangling]
    The mangled name consists of the \li{_M\$} prefix (standing for ``mangled'') followed by the name and then by the result of applying the mangle function to a type. The mangle function, for primitive types (\li{bits32}, etc.), returns their shortened name (for \li{bits32}, \li{b32}, for example), and for aggregate types, returns the prefix corresponding to the aggregate type followed by its arguments enclosed in \li{\$L} and \li{R\$} (for functions, the arguments and the returns are enclosed separately).
\end{defn}
...
\begin{description}
    ...
    \item[Block data] The mapping from each block's index and variable name relevant in the context of the corresponding block to a two-valued tuple \linebreak \li{(writes, requests alive)} where each entry is a boolean (note that in \cref{sec:blockify}, we introduced this variable as a triple, we siply strip the first value of each record away).
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \State generate $ph$, a mapping of placeholder operands for each record in the \emph{block data} without the entry block such that \li{writes} or \li{requests alive} is set
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \State translate the first statement (\cref{transl_stmt}) of $B$ \Comment{opens a LLVM basic block, variables not relevant here (label statements do not access them)}
        ...
        \For {$v$ a variable requested alive}
            ...
            \For {$(idx', idx)$ in the control flow graph}
                \State add an operand $f(idx', v)$ (if $idx'$ is the entry block) or $ph(idx', v)$ (otherwise) labeled $\mtt{blocks\ table}(idx')$ \Comment{labeled with $idx'$ block's name}
            \EndFor
