...
\begin{defn}[Primitive patterns]
    \label{def:primpat}
    Primitive patterns $\mathbb{P}$ follow the same grammar as typings (see \cref{typing_gram}) but without the very first grammar case ($\mathbb{V}$) and with type variables ($\mbb{V}$) appearing in the grammar cases in place of typings ($\mbb{T}$) wherever applicable.
...
\begin{algorithm}
    ...
    \begin{algorithmic}
        ...
        \While {$\mcal{F} \neq \emptyset$}
            ...
            \State perform the preparation step, see \cref{presteps}
            ...
            \ElsIf {$f \equiv t_1 \sqsupseteq_M t_2$}
                ...
                \EndIf
            \EndIf
            \State perform a repair step, see \cref{middlesteps}
        \EndWhile
...
\begin{enumerate}
    \item We repeatedly perform \cref{main_loop} until the resulting list $\mcal F_p$ of deferred constraints covers the whole $\mcal{F}$.
...
Note that if there was no limit on monomorphization (observe step \ref{steptwo}), it could potentially continue forever (we usually choose the limit so it is safely bigger than the monomorphization depth of majority of practical programs; this is precedented, for example, by Haskell \cite{haskell2010}):
