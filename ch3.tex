...
Flattening and blockifying phase convert a possibly nested \cmm{} source code into code with no nesting that is separated into simple blocks of continuous execution (corresponding to the target output of LLVM basic blocks \xxx{todo ref}). The output blocks form a semi-sequential representation of the code, where the basic blocks are organized in a graph structure that reflects the program flow control between the blocks. Later, this allows for a straightforward compilation to SSA language of LLVM.

LLVM basic block is defined as a sequence of consecutive statements that starts with a label (or an entry point of a procedure) and ends with a `terminator' statement, such as a branch, a return, an explicitly marked unreachable statement, or an implicit fallthrough to another basic block. Notably, basic blocks can not contain labels other than the initial ones. For \cmm, we work with even more restricted definition that only allows branch statements at the end of the basic blocks.

\subsection{Flattening}

Flattening phase replaces all nested statements by trivial single-command statements that are simpler to convert into basic blocks. In case of \cmm{}, this is achieved by transforming all \li{if} statements into labels and \li{goto} statements. An example of the transformation is shown in \cref{lst:flat}.
...
In the result, the \li{if} statements only contain a single branch expression, which may be easily translated to LLVM conditional branches. We perform the transformation similarly for \li{switch} statements. Altough \cmm{} does not specify any other control flow statements with nested bodies, the flattening would be applicable even for \li{while} statements and other, more complex conditional execution.
...
\subsubsection{Filling in calls to drop functions}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item The block translation begins by generating the list \li{vars} of variables $v$ to be imported (via phi nodes). These are the ones, for which \emph{block data} maps the blocks's \li{idx} and the variable $v$ to a tuple with the \li{requests alive} entry set.
