...
In this chapter, we show the possibilities of use of the extended \cmm compiled by the prototype compiler\cmmrepo described in \cref{chap3}.
...
In \cref{cex:cleanup}, we show that the automatic resource management features introduced by the extended \cmm can serve as an alternative to this convention. In particular, the cleanup section is replaced by a definition of an resource-representing object and accompanying \li{drop} procedure.

\paragraph{``If there is no cleanup needed then just return directly''} In order to fully serve as a practical alternative to the \li{goto cleanup} pattern in a general case, we also introduce \li{dropped} statement. \li{dropped} statement takes a named alias to the resource-representing object and the resource management action is not performed for this object on exits, which follow in the control flow of the given procedure.
...
There are multiple constructs in other languages that implement the automatic resource management action that has to happen on every scope exit. We will look at two: C++ RAII and C\# \li{try\{...\} finally\{...\}}.
