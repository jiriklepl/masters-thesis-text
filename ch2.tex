...
\begin{defn}[Subtyping system]
    We define the subtyping system as a minimization problem, where we first minimize the ``typing dimension'' (principal types \xxx{ref DM}) and then the mutually orthogonal ``subtype dimensions''.
...
\begin{ex}[Trivail use of automatic resource management]
    This example shows a common use of a resource-representing object, \li{X} can be, for example, some mutex
    \begin{lstlisting}
stackdata {
    new X;
}
