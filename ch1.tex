...
\begin{remark}[Ambiguous assignments]
...
Further, we extended the type system with a highly specialized version of subtyping, which can provide us the necessary means of type-checking and type-inferring whether a variable is semantically a compile-time constant and also what is its `data kind' (we introduce those in \cref{chap2}, which describes the language specifics).
