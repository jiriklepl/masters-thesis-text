...
Without the RAII features, we can either enclose the procedure in a wrapper which performs the necessary actions, but significantly restricting the API of the function; or we can create a cleanup section in the procedure we then use instead of regular exiting. We compare this pattern with RAII \xxx{in the next section}.


\xxx{described here and here} (namely the \li{new} specialization for structure declarations and the corresponding generation of \li{drop} calls)
\xxx{ to the reference
The RAII mechanism we propose generates, on every procedure exit, an implicit call to the cleanup code tied to the \li{new} specialized object on the stack, type of which has to be a valid argument to the \lstinline{drop} method of the \li{RAII} class.
The pointer to the object the call is tied to is then given as the argument to this call.
}
...
In order to fully serve as an alternative to the \li{goto cleanup} pattern, we also propose a special instruction \xxx{dropped}. Which takes a named alias to the RAII object and removes the implicit calls an all function exits following this instruction, then allowing for explicit calls whenever the user specifies.

This can be easily ensured by the flow analysis algorithm which is already demonstrated in the implementation. The way we propose this should be implemented is that the \xxx{dropped} instruction is illegal if there exist alternative paths in the control flow to its corresponding function exits (function exits encountered by a dfs search beginning in this instruction) which do not encounter this instruction.
...
One possible improvement over the \li{goto cleanup} pattern is that this can easily scale with more actions tied to multiple RAII objects, each such action then possibly being prevented by a separate \xxx{dropped} instruction on some subset of control flow paths.

\section{Comparison to other languages offering RAII}

There are multiple constructs that implement the RAII mechanism. We will look into two: \xxx{C++ RAII constructor/destructor} and \xxx{C\# try{}finally{...}}.
...
\begin{ex}
    Let us assume a class $C a b$ with two instances with no assumptions on some given monomorphic typings and kinds such that one requires $a >_c b$ and the other one $a <_ b$ (the instances differ only in constness and they are nonoverlapping). The sharp inequalities are not allowed in the system \xxx{ref to reasoning}, but they can be ensured by choosing two constness constants $c_1, c_2$ such that $c_1 \leq c_2$. And since the subtyping constants are assumed to be unique, it follows that $c_1 < c_2$ and therefore we can ensure two instances with the stated properties.
...
It should be clear this models the various mapping functions and sets with very complicated variables \xxx{described in where}.

And then, we recommend annotating the AST nodes with types, schemes and qualified types (schemes for polytyped top level definitions; qualified types, if the corresponding node requires some witness - in other words, if it has at least one unsolved constraint assumption; qualified types are instantiated schemes with constraints not removed from the type signature) and not just type variables, the semantics of which have to be retrieved from the inferencer state. This approach would then reduce the number of generated constraints as we do not need to generate a constraint for every single subterm (A trivial example: if we generate a type \li{bits32} for a \li{TBits 32} node, we already can generate a type \li{ptr (bits32)} for the parent node \li{TPtr (TBits 32)}). This also allows the preprocessor to directly solve some types without the need for more complicated type inference (this comes to design decisions and there are more possible ways to implement this - \xxx{comparing them is a possible concern for a future work}).
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item We collect the constraints of the form $t \to_s s$, then, we group them by $t$ and if there is any group of more than one element, we unify them. This then means that each type is mapped to exactly one subtype variable in the given subtyping dimension. We can skip this step if we keep the invariant that each type is mapped to exactly one subtype, see the step \ref{mapping_once}.

        \item We determine the strongly connected components of the graphs defined by the subtypings topology and we unify the variables present in any such strongly connected component, if the size of the corresponding component is greater than one.

        \item We then identify the lower bounds (as described by \xxx{chap 2}) of each subtype variable $s$ that is not marked.

        We then remove all constraints regarding $s$. And then we unify it with the $\sup$ constructor applied to the set of its lower bounds.

        \xxx{lemma: Then, all subtypes appearing in the constraints of the current component are either: subtype constants, or they are marked to represent skolem type constants, or they are defined via the $\sup$ constructor (then recursively for each argument, but without the $\sup$ constructor)}

        \item We add all the skolem subtype constants coming from the skolems of each context scope of the implication constraints to the quantification of this context scope, recursively. \xxx{this can be done very efficiently as the subtype skolems are marked with the original context levels of the type skolems which are bound to them}
    ...
    \item Then we look at the functions specified as \li{foreign "C"}. And report any such functions if they are not monotyped (and therefore monomorphic). Otherwise we add them to a list \li{C_symbols} of functions , names of which we do not want to mangle.
...
mangle n@(_ `Annot` (_ :: a)) = go n
  where
    ...
    Good examples of using this principle are the modules \li{CMM.Mangle} (source of the above example \xxx{ref}) and \li{CMM.FillHoles}.
